# Ajouter après la fonction create_geolocation_table() et avant la fonction get_client_ip()
...
async def create_token_history_table():
    try:
        ...
        if not table_exists:
            ...
            print("✅ Table token_history créée avec succès")
        else:
...
async def save_token_history_data(history_data):
    """
    Sauvegarde les données d'historique des tokens en base
    """
    try:
        conn = await asyncpg.connect(
            host=DB_HOST,
        ...
        for token_data in history_data:
            ...
            for history_entry in history:
                ...
                exists = await conn.fetchval(
                    'SELECT EXISTS(SELECT 1 FROM token_history WHERE token_uuid = $1 AND date = $2)',
                    token_uuid, date_obj
                )
                ...
                if exists:
                    ...
                    await conn.execute('''
                        UPDATE token_history SET
                            ...
                            token_price = COALESCE($4, token_price),
                            underlying_asset_price = COALESCE($5, underlying_asset_price),
                            initial_maintenance_reserve = COALESCE($6, initial_maintenance_reserve),
                            total_investment = COALESCE($7, total_investment),
                            gross_rent_year = COALESCE($8, gross_rent_year),
                            net_rent_year = COALESCE($9, net_rent_year),
                            rented_units = COALESCE($10, rented_units),
                            renovation_reserve = COALESCE($11, renovation_reserve)
                        WHERE token_uuid = $1 AND date = $2
                    ''',
        ...
        print(f"✅ Données d'historique sauvegardées: {insert_count} insertions, {update_count} mises à jour")
        return {"insertions": insert_count, "updates": update_count}
        
    except Exception as e:
        print(f"❌ Erreur lors de la sauvegarde des données d'historique: {str(e)}")
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail="Erreur lors de la sauvegarde des données d'historique")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code != 200:
                ...
                raise HTTPException(status_code=500, detail=f"Erreur lors de la récupération des données: {response.status_code}")
...
async def startup():
    try:
        ...
        await create_leaderboard_table()
        print("✅ Table de géolocalisation vérifiée avec succès")
        await create_minigames_table()
        print("✅ Table minigames vérifiée avec succès")

        await create_exploration_locations_table()
        await seed_initial_exploration_locations()

    except Exception as e:
        print(f"⚠️ Erreur lors de la création/vérification des tables: {str(e)}")
        print("⚠️ L'API fonctionnera mais certaines fonctionnalités pourraient ne pas être disponibles")
...
async def clean_metrics_geolocation(request: Request):
    ...
    try:
        ...
        # Vérifier si conn existe avant de le fermer
        if conn is not None:
            await conn.close()

# ------------------------------------------------------------------------------
# Endpoints pour l'API Token History
# ------------------------------------------------------------------------------
...
async def sync_token_history(request: Request):
    ...
    try:
        ...
        await update_execution_time("token_history_sync")
...
async def update_execution_time(request_id: str):
    ...
    try:
        ...
        # Vérifier si conn existe avant de le fermer
        if conn is not None:
            await conn.close() # Ajouts pour l'historique des tokens - À intégrer dans main.py
